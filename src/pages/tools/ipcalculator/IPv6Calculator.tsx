...
export default function IPv6Calculator() {
    ...
    const [ipv6Address, setIPv6Address] = useState<IPv6Address>({
        compressed: '2001:4860::8888',
        full: '2001:4860:0000:0000:0000:0000:0000:8888',
        decimal: '4254195612376984603617138956568135816'
    });
    ...
    const handleIPv6Change = (value: string) => {
        ...
        if (isValidIPv6(value)) {
            ...
            const decimal = ipv6ToDecimal(value);
    ...
    const handleDecimalChange = (value: string) => {
        ...
        const ipv6 = decimalToIPv6(value);
        if (ipv6) {
            const expanded = expandIPv6(ipv6);
            setIPv6Address({
                compressed: ipv6,
    ...
    return (
        <Card
            title="IPv6地址计算器"
            size="small"
            headStyle={{ background: '#fff1f0', fontSize: '14px' }}
                    ...
                    <Row gutter={4} style={{ marginTop: '6px' }}>
                        ...
                        <Col span={8}>
                            <Input
                                ...
                                placeholder="完整地址"
                                size="small"
                                style={{ background: '#f5f5f5', fontSize: '10px' }}
                            />
                        </Col>
                    </Row>
                </div>
                        ...
                        <Row gutter={4} style={{ marginTop: '8px' }}>
                            <Col span={12}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Text strong style={{ fontSize: '11px', color: '#666' }}>起始地址</Text>
                                    <Text code copyable style={{ fontSize: '10px' }}>{ipv6Range.range.start}</Text>
                                </div>
                            </Col>
                            <Col span={12}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Text strong style={{ fontSize: '11px', color: '#666' }}>结束地址</Text>
                                    <Text code copyable style={{ fontSize: '10px' }}>{ipv6Range.range.end}</Text>
                                </div>
                            </Col>
                        </Row>
                    )}
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '11px', lineHeight: '1.6', color: '#555' }}>
                            ...
                            <p style={{ margin: '0 0 8px 0' }}>
                                ...
                                <Text code style={{ fontSize: '10px', background: '#f0f0f0' }}>xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx</Text>
                            </p>

                            <p style={{ margin: '0 0 8px 0' }}>
                                类似于 IPv4中的CDIR表示法，IPv6用前缀来表示网络地址空间，比如：
                                <Text code style={{ fontSize: '10px', background: '#f0f0f0' }}>2001:251:e000::/48</Text>
                                表示前缀为48位的地址空间，其后的80位可分配给网络中的主机，共有2的80次方个地址。
                            </p>

                            <Text strong style={{ fontSize: '11px', color: '#666', display: 'block', margin: '12px 0 8px 0' }}>
                                IPv6地址的表示方法
                            </Text>
                            ...
                            <div style={{ marginLeft: '12px', marginBottom: '8px' }}>
                                <Text strong style={{ fontSize: '11px', color: '#666' }}>（1）冒分十六进制表示法</Text>
                                <p style={{ margin: '4px 0', fontSize: '10px' }}>
                                    格式为X:X:X:X:X:X:X:X，其中每个X表示地址中的16b，以十六进制表示，例如：
                                </p>
                                <Text code style={{ fontSize: '10px', background: '#f0f0f0', display: 'block', margin: '4px 0' }}>
                            ...
                            <div style={{ marginLeft: '12px', marginBottom: '8px' }}>
                                <Text strong style={{ fontSize: '11px', color: '#666' }}>（2）0位压缩表示法</Text>
                                <p style={{ margin: '4px 0', fontSize: '10px' }}>
                            ...
                            <div style={{ marginLeft: '12px' }}>
                                <Text strong style={{ fontSize: '11px', color: '#666' }}>（3）内嵌IPv4地址表示法</Text>
                                <p style={{ margin: '4px 0', fontSize: '10px' }}>
                                    为了实现IPv4-IPv6互通，IPv4地址会嵌入IPv6地址中，此时地址常表示为：
                                    X:X:X:X:X:X:d.d.d.d，前96b采用冒分十六进制表示，而最后32b地址则使用IPv4的点分十进制表示。
                                </p>
                                <p style={{ margin: '4px 0', fontSize: '10px' }}>
                                    例如<Text code style={{ fontSize: '10px', background: '#f0f0f0' }}>::192.168.0.1</Text>与
                                    <Text code style={{ fontSize: '10px', background: '#f0f0f0' }}>::FFFF:192.168.0.1</Text>
